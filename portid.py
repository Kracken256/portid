#!/usr/bin/python3
# portid copyright Wesley Jones 2022 ©
...
def print_line(line: str):
    ...
    desc = ','.join(data[2:]).replace("\"", '')
    print(f"{data[1]}/{data[0].lower()[1:-1].strip()}", f"{desc}".strip())


def lookup(port: str, type: str):
    content = databank.split("\n")
    if type != None:
        for line in content:
            data = line.strip().split(",")
            if data[0].replace("\"", "").upper() == type and data[1] == port:
                print_line(line)
                return
    else:
        for line in content:
...
def print_help():
    print("Tool by Kracken256")
    print("Supports TCP and UDP ports")
    print("Usage:")
    print("\tportid [port-number/protocol]")
    print("Example 1:")
    print("\tportid 443/tcp")
    print("Example 2:")
    print("\tportid 3306")
...
def main():
    if "-h" in sys.argv or "--help" in sys.argv or len(sys.argv) == 1:
        print_help()
        exit()
    arg = sys.argv[1].upper().split("/")
    if len(arg) == 1:
        lookup(arg[0],None)
    elif arg[0].isnumeric() and (arg[1] == "TCP" or arg[1] == "UDP"):
        lookup(arg[0], arg[1])
        exit()
    return
